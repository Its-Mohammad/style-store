...
function Women() {
  ...
  const womenCategory = categories.find((category) => category.id === "women");
  ...
  const availableColors = useMemo(() => {
    return getAvailableColors(womenProducts);
  }, [womenProducts]);

  const availableSizes = useMemo(() => {
    return getAvailableSizes(womenProducts);
  }, [womenProducts]);

  const filteredProducts = useMemo(() => {
    return filterAndSortProducts(womenProducts, {
      selectedSubcategory,
      selectedColor,
      selectedSize,
      onlyDiscounted,
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-12">
      <div className="mb-10">
        <p className="text-sm font-bold text-neutral-500">Women Collection</p>

        <h1 className="mt-2 text-4xl font-bold md:text-5xl">پوشاک زنانه</h1>

        <p className="mt-5 max-w-2xl text-lg leading-8 text-neutral-400">
          کالکشن زنانه با تمرکز روی فرم، راحتی، رنگ‌های مینیمال و استایل مدرن
          شهری.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[240px_1fr]">
        <ProductFilterPanel resultCount={filteredProducts.length}>
          <ProductFilters
            categories={womenCategory.subcategories}
